...
class Yearbook extends CI_Controller {
	...
	public function add_yearbook_data()
	{
		...
		if(!isset($sessiontrue))
			...
			$this->form_validation->set_rules('book_year', 'Book Year', 'required');
			$this->form_validation->set_rules('book_description', 'Book Description', 'required');
			...
			$this->form_validation->set_rules('ceo_quote', 'CEO Quote', 'required');
				...
				$this->upload->do_upload('yearbookimg');

					$data = array('yearbook_name'    => $_POST['book_name'],
						          'slug'    => url_title($_POST['book_name'], 'underscore', TRUE),
						          'yearbook_year'    => $_POST['book_year'],
						          'yearbook_description'    => $_POST['book_description'],
						          "yearbook_image"    =>$_FILES['yearbookimg']['name'],
		...
		if(!isset($sessiontrue))
			...
			else
			{
				...
				$this->upload->do_upload('ceoimg');
			     ...
			     if($_FILES['yearbookimg']['error'] ==4)
				{
				
				 $yearbookimg = $_POST['old_yearbookimg'];
			    }
			    else
			    {
			    $config['upload_path']          = 'uploads/yearbook/';
				...
				$this->upload->do_upload('yearbookimg');
				 ...
				 $yearbookimg = $_FILES['yearbookimg']['name'];
			    }
